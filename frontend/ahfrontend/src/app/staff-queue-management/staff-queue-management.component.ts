...
function toLocalMonthInputValue(date: Date): string {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, '0');
  return `${year}-${month}`;
}
...
export class StaffQueueManagementComponent implements OnInit, OnDestroy {
  ...
  overviewDate = signal<string>(toLocalDateInputValue(new Date()));
  overviewMonth = signal<string>(toLocalMonthInputValue(new Date()));
